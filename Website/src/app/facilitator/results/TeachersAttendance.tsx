...
export default function TeachersAttendance() {
  ...
  // UI State
  const [activeTab, setActiveTab] = useState<"teachers" | "assignments" | "overview">("overview");
  ...
  const addAssignment = async () => {
    if (!newAssignment.teacherId || !newAssignment.subjectId || !newAssignment.grade || !newAssignment.academicYear) {
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <nav className="flex space-x-4 border-b">
          <button
            ...
            className={`py-2 px-4 font-medium ${activeTab === "overview" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-600 hover:text-blue-600"}`}
          >
            Overview
          </button>
          <button
            onClick={() => setActiveTab("teachers")}
            className={`py-2 px-4 font-medium ${activeTab === "teachers" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-600 hover:text-blue-600"}`}
          >
            Teachers
          </button>
          <button
            onClick={() => setActiveTab("assignments")}
            className={`py-2 px-4 font-medium ${activeTab === "assignments" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-600 hover:text-blue-600"}`}
          >
            Assignments
          </button>
        </nav>
                ...
                <table className="min-w-full border-collapse border">
                  ...
                  <tbody>
                    {assignments.slice(0, 5).map((assignment) => (
                      <tr key={assignment._id || `${assignment.teacherId}-${assignment.subjectId}`} className="hover:bg-gray-50">
                ...
                <tbody>
                  {teachers.map((teacher) => (
                    <tr key={teacher._id} className="hover:bg-gray-50">
                      ...
                      <td className="border p-3">{teacher.specialization || "-"}</td>
                      <td className="border p-3">
                        <span className={`px-2 py-1 rounded text-sm ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {newAssignment.grade && getSubjectsByGrade(newAssignment.grade).map(subject => (
