...
export default function AdminDashboard() {
  ...
  const filteredStudents = students.filter((s) =>
    [s.Unique_ID, s.First_Name, s.Father_Name, s.Grade, s.Sex]
      .join(" ")
      .toLowerCase()
  ...
  const pagedFacilitators = filteredFacilitators.slice(
    (facilitatorPage - 1) * PAGE_SIZE,
    facilitatorPage * PAGE_SIZE
  );
  ...
  return (
    <div className="min-h-screen flex flex-col gap-8">
      ...
      <div className="bg-white shadow rounded-xl p-4 md:p-6 mb-8">
        ...
        <div className="overflow-x-auto max-h-[400px]">
          <table className="min-w-full border-collapse border text-sm md:text-base">
            <thead className="bg-gray-100">
              <tr>
                <th className="border p-2 md:p-3 text-left">ID Number</th>
                <th className="border p-2 md:p-3 text-left">Name</th>
                <th className="border p-2 md:p-3 text-left">Grade</th>
                <th className="border p-2 md:p-3 text-left">Sex</th>
              </tr>
            </thead>
            <tbody>
              {pagedStudents.map((student) => (
                <tr key={student._id?.toString()} className="hover:bg-gray-50">
                  <td className="border p-2 md:p-3">{student.Unique_ID}</td>
                  <td className="border p-2 md:p-3">{`${student.First_Name} ${student.Father_Name}`}</td>
                  <td className="border p-2 md:p-3">{student.Grade}</td>
                  <td className="border p-2 md:p-3">{student.Sex}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* Pagination */}
        <div className="flex gap-2 mt-4 justify-center text-sm md:text-base">
          <button
      ...
      <div className="bg-white shadow rounded-xl p-4 md:p-6 mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              className="bg-blue-600 text-white px-3 md:px-4 py-2 rounded-lg hover:bg-blue-700 text-sm md:text-base"
              ...
              {pagedFacilitators.map((fac: User) => (
                <tr key={fac._id?.toString()} className="hover:bg-gray-50">
                  ...
                  <td className="border p-2 md:p-3 flex flex-wrap gap-2">
                    ...
                    <button
                      className="bg-red-600 text-white px-2 md:px-3 py-1 rounded hover:bg-red-700 text-sm md:text-base"
        ...
        <div className="flex gap-2 mt-4 justify-center text-sm md:text-base">
          ...
          <span className="px-2">
            Page {facilitatorPage} of {facilitatorPages}
          </span>
          <button
            className="px-3 py-1 rounded border bg-gray-100"
            onClick={() =>
              setFacilitatorPage((p) => Math.min(facilitatorPages, p + 1))
            }
